<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>MM Enterprise</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <link rel="stylesheet" href="../public/css/bienvenido.css">
</head>
<body>
    <div class="sidebar fw-bold">
        <div class="logo">
        MM Enterprise
        </div>
        <a href="ventas.php" class="menu-item">Ventas</a>
        <a href="afiliados.php" class="menu-item">Afiliaciones</a>
        <a href="inventario.php" class="menu-item">Inventario</a>
        <a href="despachos.php" class="menu-item">Despachos</a>
        <a href="productos.php" class="menu-item">Productos</a>
    </div>

    <div class="main-content d-flex align-items-center">
        <div class="welcome-section my-auto mx-auto text-center">
            <h1 class="fw-bold">Bienvenido a MM</h1>
            <p>Seleccione una opcion del menu lateral</p>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    </body>
</html>